...
const Relationship = (props) => (
    <div className='p-3'>
        <input type='text' name="relationship_kid" defaultValue={props.selectedKid.relationships[props.row]?.kid} className='sm:block sm:w-full sm:mb-1 md:inline md:w-1/5 mr-1' onChange={(evt) => props.updateRelationship(props.row, 'kid', evt.target.value)} />
        <input type='text' name="relationship_relationship" defaultValue={props.selectedKid.relationships[props.row]?.relationship} className='sm:block sm:w-full md:inline md:w-3/4 mr-1' onChange={(evt) => props.updateRelationship(props.row, 'relationship', evt.target.value)} />
